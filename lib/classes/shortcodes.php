		...
		if ( false === ( $content = get_transient( 'auction_highlights_' . $auction ) ) || true == $flushcache ) {
			...
			if( $posts ){
				...
				foreach( $posts as $post ){
					...
					$content[] = '<div class="highlight clearfix"><div class="first one-fourth"><a href="' . get_the_permalink() . '" title="' . esc_attr( get_the_title() ) . '">' . $image . '</a></div><div class="three-fourths"><h3><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h3>'.apply_filters( 'the_content', get_the_content() . $item_meta ).'</div></div>';
				}
			} else {
