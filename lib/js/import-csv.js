...
function importCSV( id, offset ){
	...
	jQuery.ajax({
		...
		type: 'POST',
		dataType: 'json',
		data: { action: 'auction_importer', cb_action: 'import_csv', csvID: id, csvoffset: offset },
		timeout: 25000,
		success: function( response ) {
			var data = response.data;
			console.log( response );
			if( jQuery('#import-progress h2').html() == '' ) jQuery('#import-progress h2').html('Importing <em>' + data['title'] + '</em>');
			console.log( '[ImportCSV] offset = ' + offset + ', ' + data['current_offset'] + ', images = ' + data['images'] );

			// Attach images to the auction item
			if( data['images'] instanceof Array ) {
				var images = data['images'];
				for( i = 0; i < images.length; i++ ) {
					importImage( data['post_ID'], images[i], data['imgdir'] );
				}
			}

			var rows = data['csv']['rows'];
			var html = '';

			if(data['selected_rows'] > 0){
				jQuery('#import-stats').html('(' + data['current_offset'] + ' out of ' + data['total_rows'] + ')');

				var width = percent( offset, data['csv']['row_count'], 0, 100 );
				jQuery('#import-percent').html( width + '%' );
				width = 2 * Math.round( width );
				jQuery( '#import-progress-bar' ).width( width ); // adjust the width of the progress bar
				importCSV( data['id'], data['offset'] );
			} else {
				jQuery('#import-percent').html('Import complete!');
				jQuery('#import-progress-bar').css( 'background-image', 'none' );
				jQuery('#import-progress-bar').css( 'border', 'none' );
