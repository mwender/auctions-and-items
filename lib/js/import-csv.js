...
function importCSV( id, offset ){
	...
	var jqXHR = jQuery.post( ajax_vars.ajax_url, data, function(response){
		...
		console.log( '[ImportCSV] ERROR:', response.data.message );
		jQuery('#import-progress-container').fadeOut();
		jQuery('#import-note').html( response.data.message );
		console.log( '⚠️ errorObj: ', errorObj );
		if( 'timeout' == error ) {
...
function loadCSV(id){
	...
	jQuery.post( ajax_vars.ajax_url, data, function(response){
		...
		let displayColumns = ['ItemNumber', 'LotNumber', 'Lead', 'Description', 'ProvenanceLine', 'Condition']; // 02/24/2025 (10:06) - Case Sensitive
		var headings = '';

		const columnsIndexes = [];
		for( var i = 0; i < displayColumns.length; i++ ){
			columnsIndexes.push( columns.indexOf( displayColumns[i] ) );
			headings += '<th scope="col" class="manage-column">' + displayColumns[i] + '</th>' + "\n";
		}
		jQuery(headings).appendTo(jQuery('#csvimport thead tr'));

		var rows = response.csv['csv']['rows'];
		var html = '<tr class="alternate"><td colspan="' + displayColumns.length + '" style="text-align: center;"><strong>Auction:</strong> ' + response.csv['auction'] + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>Image Folder:</strong> /' + response.csv['imgpath'] + '/</td></tr>';

		let counter = ( rows.length < 5 )? rows.length : 5 ;
		...
		for(var i = 0; i < counter; i++){
			...
			for(j = 0; j < displayColumns.length; j++){
				let currentIndex = columnsIndexes[j];
				let value = ( typeof row[currentIndex] == 'undefined' )? '&nbsp;' : row[currentIndex].substring(0,100);
				if( typeof row[currentIndex] == 'undefined' ){
					value = '&nbsp;';
				} else if( row[currentIndex].length > 100 ){
					value = row[currentIndex].substring(0,100) + '[...]';
				} else {
